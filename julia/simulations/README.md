...
The primary command surface is `bin/stenosis-hemodynamics <command>`.
It internally uses `bin/julia-release`, which prefers the latest installed
...
```bash
bin/stenosis-hemodynamics simulate --tfinal 0.01 --nx 120 --ic-pressure-drop-pa 40 --ic-mesh-nz 2 --ic-mesh-nr 2 --ic-mesh-ntheta 8 --progress-every 0 --output julia/simulations/output/verification_001s.csv --svg julia/simulations/output/verification_001s.svg
```

Default 50% stenosis native run:

```bash
bin/stenosis-hemodynamics simulate --tfinal 1.0 --nx 400 --severity 50 --ic-pressure-drop-pa 40 --progress-every 10000
```

Legacy first-order run:

```bash
bin/stenosis-hemodynamics simulate --space fv-first-order --ic geometry-rest --tfinal 0.001 --nx 80 --progress-every 0
```

DG quadratic smoke run:

```bash
bin/stenosis-hemodynamics simulate --space dg --degree 2 --time-stepper ssprk3 --ic-pressure-drop-pa 40 --ic-mesh-nz 2 --ic-mesh-nr 2 --ic-mesh-ntheta 8 --tfinal 0.001 --nx 80 --progress-every 0
```

Carreau-Yasuda smoke run:

```bash
bin/stenosis-hemodynamics simulate --tfinal 0.001 --nx 80 --severity 40 --ic geometry-rest --rheology carreau-yasuda --eta0 0.56 --eta-inf 0.0345 --lambda-s 3.313 --yasuda-a 2.0 --flow-index 0.3568 --progress-every 0
```
...
```bash
bin/stenosis-hemodynamics simulate --backend sciml --alg auto --ic geometry-rest --tfinal 0.001 --nx 40 --progress-every 0
bin/stenosis-hemodynamics simulate --backend sciml --alg tsit5 --abstol 1e-7 --reltol 1e-7 --ic geometry-rest --tfinal 0.001 --nx 40 --progress-every 0
bin/stenosis-hemodynamics simulate --backend sciml --alg rodas5p --maxiters 1000000 --ic geometry-rest --tfinal 0.001 --nx 40 --progress-every 0
```
...
```bash
bin/stenosis-hemodynamics simulate --help
```

Run small studies through the command dispatcher:

```bash
bin/stenosis-hemodynamics study severity --severities 23,50 --nx 40 --tfinal 0.001 --ic geometry-rest --overwrite
bin/stenosis-hemodynamics study grid --nxs 40,80 --severity 50 --tfinal 0.001 --ic geometry-rest --overwrite
bin/stenosis-hemodynamics study refinement --nxs 50,100,200,400 --severity 40 --tfinal 0.001 --ic geometry-rest --overwrite
bin/stenosis-hemodynamics stokes refine --nx 80 --parallel-workers 1 --overwrite
```

Study summaries are written to deterministic CSV paths under
`julia/simulations/output/` by default. Existing summary files are not overwritten
...
`julia/simulations/output/stationary_stokes_refinement/` by default and include mesh
...
LaTeX table fragments under `julia/simulations/output/refinement/<study-token>/`.
...
profile diagnostics under `julia/simulations/output/3d_comparison/`.
...
```text
julia/simulations/data/3d/canic_case3/77/velocity.xdmf
julia/simulations/data/3d/canic_case3/77/velocity.h5
julia/simulations/data/3d/canic_case3/60/velocity.xdmf
julia/simulations/data/3d/canic_case3/60/velocity.h5
```
...
```bash
mkdir -p julia/simulations/data/3d/canic_case3
cp -R /path/to/case3_all_3d_results/77 julia/simulations/data/3d/canic_case3/
cp -R /path/to/case3_all_3d_results/60 julia/simulations/data/3d/canic_case3/
```

Run the default comparison when the data root is present:

```bash
bin/stenosis-hemodynamics compare-3d \
  --target-time 0.9995 \
  --time-atol 1e-6 \
  --overwrite \
...
```bash
bin/stenosis-hemodynamics compare-3d --backend sciml --alg tsit5 --abstol 1e-7 --reltol 1e-7 --overwrite
```
