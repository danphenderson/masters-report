@testset "StenosisHemodynamics operator validation workflow" begin
    mktempdir() do dir
        spec = StenosisHemodynamics.OperatorValidationSpec(
            output_dir=dir,
        ...
        @test StenosisHemodynamics.workflow_kind(spec) == "cross_section_operator_validation"
        @test StenosisHemodynamics.default_output_paths(spec).summary_csv ==
              joinpath(dir, "cross_section_operator_validation.csv")

        result = StenosisHemodynamics.run_operator_validation(spec)
    ...
    @test_throws ArgumentError StenosisHemodynamics.validate_workflow_spec(
        StenosisHemodynamics.OperatorValidationSpec(sample_z_cm=[1.0]),
    )
    @test_throws ArgumentError StenosisHemodynamics.OperatorValidationSpec(affine_coefficients=[1.0, 2.0, 3.0])
end
