function print_usage(io::IO = stdout)
    println(
        ...
        """
        Usage:
          bin/stenosis-hemodynamics <command> [options]
        ...
        Run 'bin/stenosis-hemodynamics <command> --help' for command help.
        """,
    )
end
...
function print_simulate_usage(io::IO = stdout)
    println(
        ...
        """
        Usage:
          bin/stenosis-hemodynamics simulate [options]
...
function print_openbf_usage()
    println("""
    Usage:
      bin/stenosis-hemodynamics openbf-run --config PATH [--verbose] [--save-stats]
    """)
end
...
function print_benchmark_usage()
    println("""
    Usage:
      bin/stenosis-hemodynamics benchmark --profile smoke|overnight --output-dir PATH [--overwrite]
    """)
end
...
function benchmark_spec_from_cli(args::Vector{String})
    ...
    return PackageBenchmarkSpec(;
        ...
        output_dir=get(values, "output-dir", joinpath("julia", "simulations", "output", "package_benchmark", "smoke")),
...
function print_study_usage()
    println("""
    Usage:
      bin/stenosis-hemodynamics study severity --severities 23,50 [options]
      bin/stenosis-hemodynamics study grid --nxs 40,80 [options]
      bin/stenosis-hemodynamics study refinement --nxs 50,100,200,400 [options]
    """)
end
...
function print_stokes_usage()
    println("""
    Usage:
      bin/stenosis-hemodynamics stokes refine [--severities 0,23,40,50] [--meshes 8x2x8,16x4x16] [options]
    """)
end
...
function print_verify_usage()
    println("""
    Usage:
      bin/stenosis-hemodynamics verify mms [--nxs 20,40,80] [--dt-values 2e-5,1e-5,5e-6] [options]
      bin/stenosis-hemodynamics verify ph-refinement [--h-nxs 20,40,80,160] [--degrees 0,1,2,3,4] [options]
      bin/stenosis-hemodynamics verify rest [--severities 23,40] [--nxs 50,100,200] [--elapsed-times 0,0.001,0.005] [options]
...
function run_verify_cli(args::Vector{String})
    ...
    output_dir = get(values, "output-dir", joinpath("julia", "simulations", "output", "verification"))
    overwrite = "overwrite" in flags
    progress_every = parse(Int, get(values, "progress-every", "0"))
...
function print_compare3d_usage()
    println("""
    Usage:
      bin/stenosis-hemodynamics compare-3d [--data-root PATH] [--output-dir PATH] [--target-time SECONDS] [--time-atol SECONDS] [--overwrite] [--publish-report-assets]
      bin/stenosis-hemodynamics compare-3d --nxs 200,400,800 [--data-root PATH] [--output-dir PATH] [--target-time SECONDS] [--time-atol SECONDS] [--overwrite] [--publish-report-assets]
      bin/stenosis-hemodynamics compare-3d --nxs 200,400,800 --reuse-grid-summary PATH [--grid-summary-csv PATH] [--grid-summary-tex PATH] [--overwrite]
    """)
end
...
function print_operator_validation_usage()
    println("""
    Usage:
      bin/stenosis-hemodynamics operator-validation [--output-dir PATH] [--summary-csv PATH] [--summary-tex PATH] [--sample-z LIST] [--plane-center CM] [--plane-shifts LIST] [--constant-value CM_PER_S] [--affine-coefficients C0,CX,CY,CZ] [--tolerance VALUE] [--overwrite]
    """)
end
