...
Sec.~\ref{subsubsec:cross-sectional-reduction-selected-1d-model} and the
comparison boundary in Sec.~\ref{subsubsec:model-hierarchy}. It is supporting
method detail, not a claim that the current solver has external-solver parity.
...
The reduced state is $\mathbf U=(A,Q)^\top$. For the classical elastic
balance-law contract, the reference elastic-potential form is
\begin{flalign*}
    \quad
    ...
    \partial_t Q
    +
    \partial_z\left(\frac{Q^2}{A}+\Psi(A,z,t)\right)
    &=
...
\begin{flalign*}
    ...
    \partial_A\Psi(A,z,t)
    =
    \frac{A}{\rho}\partial_Ap_{\mathrm g}(A,z,t), &&
\end{flalign*}
...
\begin{flalign*}
    ...
    S_{\mathrm{geom}}(A,z,t)
    =
    \left.\partial_z\Psi(A,z,t)\right|_A
    -
    \frac{A}{\rho}
    \left.\partial_zp_{\mathrm g}(A,z,t)\right|_A. &&
\end{flalign*}
...
\end{flalign*}
The strictly hyperbolic positive-area region for this classical model has wave
speeds
\begin{flalign*}
    \quad
    \lambda_\pm(A,Q;z)
    ...
    \frac{Q}{A}\pm c(A,z). &&
\end{flalign*}
The boundary-state construction is intended for the subcritical case
$\lambda_-<0<\lambda_+$, so one datum enters at each end of the vessel.
...
\todo[inline]{ADVISOR[BLOCKER]: This reference balance law omits the
momentum-flux correction \(\alpha\) used in Eq.~\ref{eq:1d-momentum-aq} and
Definition~\ref{def:1d-conservative-flux-source-notation}. Either insert
\(\alpha Q^2/A\) and the corresponding \(\alpha\)-dependent wave speeds, or
state explicitly that this subsection is the special \(\alpha=1\) reference
contract.}

\subsection{Current semi-discrete solver operator}
\label{app:num-current-solver-operator}

The current Canic extended implementation uses the code variable \(A\) for a
radius-squared area coordinate. In this subsection write that coordinate as
\(a=R^2\). The physical circular cross-sectional area is
\(A_{\mathrm{phys}}=\pi a\). This is a local convention for the active solver
record; the continuum and classical 1D model sections keep \(A\) for physical
...
\begin{flalign*}
    ...
    +\widehat S_i. &&
\end{flalign*}
All divisions by \(a\) in the implementation use the recorded positive-area
floor.

With \(K=Eh/(1-\sigma^2)\) and
\(\alpha_c(R_0')=-2(R_0')^2/35\), the current full-state flux is
\begin{flalign*}
    \quad
    \mathbf F(a,Q,z)
    =
    \left(
        Q,\,
        \left(\alpha+\alpha_c(R_0'(z))\right)\frac{Q^2}{a}
...
\end{flalign*}
where \(\mathbf U=(a,Q)^\top\) and the present implementation uses neighboring
first-order states plus ghost boundary states. The recorded local speed is
\begin{flalign*}
    \quad
    s_{i+1/2}
    =
...
The source term uses the recorded centered interior and one-sided boundary
difference \(D_z^h\) on the cell averages. Let
\(g_\alpha=\alpha/(\alpha-1)\),
\[
    \dot\gamma_{1D,i}=g_\alpha\frac{|Q_i|}{a_i\sqrt{a_i}},
    \qquad
...
\begin{flalign*}
    ...
    \widehat S_i
    &=
    -2\nu_{\mathrm{eff},i}^{\mathcal R}g_\alpha\frac{Q_i}{a_i}
    +
    \frac{K}{\rho R_{\max}^2}a_iR_0'(z_i)
    -
    a_i\,\widehat{\partial_z p_2}_i
    +
    \frac{Q_i^2}{a_i}\alpha_c'(z_i), &&\\
    \quad
    \widehat{\partial_z p_2}_i
    &=
    \nu_{\mathrm{eff},i}^{\mathcal R} g_\alpha
    \left[
...
the code applies the recorded shear-rate floor and optional minimum/maximum
dynamic viscosity clamps before dividing by density.

\subsection{Time and backend realization}
\label{app:num-time-backend-realization}

The native backend advances the semi-discrete operator with the recorded
...
\begin{flalign*}
    ...
    \right\}. &&
\end{flalign*}
The SciML backend constructs an \texttt{ODEProblem} for the same
\(\dot u_h=\mathcal L_h(u_h,t;\theta)\) and applies the selected
OrdinaryDiffEq policy recorded in the solve specification. Thus backend
comparisons change the time integrator, not the semi-discrete spatial
operator.

\subsection{Boundary-state realization}
\label{app:num-boundary-state-realization}

The current finite-volume record imposes one incoming datum at each boundary in
the subcritical regime using the code's Riemann-invariant construction. Let
\begin{flalign*}
    \quad
