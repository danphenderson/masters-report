...
The implemented report-now spatial surface also includes a native
...
\begin{flalign*}
    ...
    \qquad -1\le\xi\le1,\qquad p\in\{0,1,2\}. &&
\end{flalign*}
...
cell-average finite-volume bridge; degrees one and two are native modal-DG
benchmark rows, not the selected C23/C40 comparison method.
...
\begin{flalign*}
    ...
    u^{n+1} &= \Pi\left(
        ...
        \right]\right). &&
\end{flalign*}
The native code also exposes Forward Euler and SSPRK2 steppers for diagnostic
sensitivity rows; they are not the selected time integrators for the principal
comparison. The native step size is
\begin{flalign*}
    \quad
    \Delta t_n
    =
...
surface exposes \texttt{auto}, \texttt{tsit5}, and \texttt{rodas5p}. Thus
