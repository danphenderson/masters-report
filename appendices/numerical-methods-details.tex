...
method detail, not a claim that the active solver record has external-solver
parity.
...
characteristic enters through the outlet, and one scalar boundary datum is
...
\subsection{Recorded semi-discrete solver operator}
\label{app:num-current-solver-operator}

The active solver record uses the code variable \(A\) for a radius-squared area
coordinate. In this subsection write that coordinate as \(a=R^2\). The
...
\begin{flalign*}
    ...
    +\widehat S_i. &&
\end{flalign*}
All divisions by \(a\) in the active solver record use the recorded positive-area
floor.

With \(K=Eh/(1-\sigma^2)\), velocity-profile momentum factor
\(\alpha_{\mathcal P}\), and
\(\alpha_c(R_0')=-2(R_0')^2/35\), the recorded full-state flux is
\begin{flalign*}
    \quad
    \mathbf F(a,Q,z)
    =
...
\end{flalign*}
where \(\mathbf U=(a,Q)^\top\) and the active solver record uses neighboring
...
The active finite-volume record imposes one incoming datum at each boundary in
