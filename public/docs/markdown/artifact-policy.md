...
| Large/raw data | `public/var/data/simulations/**`, local XDMF/HDF5 resolved-3D inputs, future raw solver dumps or external datasets | Keep broad raw data ignored. Track only approved data-release inputs, currently the Canic case3 XDMF/HDF5 files under `public/var/data/simulations/canic_case3/**`, with checksums recorded in `report/assets/data/canic-replication/canic-section41-provenance.json`. | Do not regenerate external raw inputs. Regenerate derived data only when regeneration is documented. | Delete only disposable local copies with known source or archive pointers. | Record source, checksum, and expected local path before relying on the data. Keep any unapproved raw data out of ordinary commits until an archival strategy is approved. |
...
`public/final-report.pdf` only after the gate passes. That path is ignored in
the source-tree candidate, so do not stage a refreshed PDF in ordinary source
validation.
