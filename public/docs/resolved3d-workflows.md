...
and the task explicitly wants deformed-coordinate plane cuts; the retained
report assets currently use the reference-coordinate velocity comparison.
...
Run the scratch membrane-FSI workflow when the task needs a Canic-style
coupled-wall comparator generated by the package:
...
per-case history CSV, and a manifest under the output directory. Dynamic wall
mode is a reduced radial membrane time integrator coupled to repeated
...
Publish these outputs to report assets only in an explicit artifact-refresh
lane.
