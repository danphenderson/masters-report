...
def test_references_inventory_allows_public_metadata_without_tracked_full_text() -> None:
    repo = Path(__file__).resolve().parents[1]
    check_archive_coverage = runpy.run_path(str(repo / "scripts/audit_references.py"))["check_archive_coverage"]
    rows = [
        {
            "_line": "2",
            "source_id": "example",
    ...
    issues = check_archive_coverage(Path.cwd(), rows, set(), {"KnownKey"}, {"KnownKey"})
