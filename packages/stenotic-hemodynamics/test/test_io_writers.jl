@testset "StenoticHemodynamics internal I/O writer helpers" begin
    mktempdir() do dir
        ...
        StenoticHemodynamics.write_csv_table(
            csv_path,
            ("label", "value", "flag"),
            rows;
            overwrite=false,
        ...
        @test read(csv_path, String) ==
              ...
              "\"needs,quotes\",,false\n" *
              "\"quote \"\"mark\"\"\",NaN,\"line\nbreak\"\n"
        @test_throws ArgumentError StenoticHemodynamics.write_csv_table(
            csv_path,
            ("label",),
            [("replacement",)];
            overwrite=false,
        ...
        @test_throws ArgumentError StenoticHemodynamics.write_json(json_path, Dict("a" => 2); overwrite=false)

        digest = StenoticHemodynamics.sha256_file(csv_path)
