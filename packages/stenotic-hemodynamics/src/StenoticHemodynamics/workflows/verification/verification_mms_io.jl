...
function write_manufactured_verification_tex(
    ...
    guarded_open_write(path, overwrite) do io
        ...
        println(io, "    \\caption{Manufactured-solution spatial verification errors in the cell-center discrete \$L_1\$, \$L_2\$, and \$L_\\infty\$ metrics. The final column checks the inserted forcing against a separately expanded residual expression that still shares the manufactured states, parameters, and lower-level utilities.}")
