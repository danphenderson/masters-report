...
function initial_condition_values(::ManufacturedSolutionIC, p::Params, z::Vector{Float64})
    p.forcing isa ManufacturedForcing ||
        throw(ArgumentError("ManufacturedSolutionIC requires forcing=ManufacturedForcing(...)"))
    A = [manufactured_area(p.forcing, zi, 0.0, p) for zi in z]
    Q = [manufactured_flow(p.forcing, zi, 0.0, p) for zi in z]
    return A, Q, InitialConditionSummary("manufactured-solution")
end
