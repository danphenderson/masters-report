...
REFERENCE_ROOT = Path("public/references")
INVENTORY_PATH = REFERENCE_ROOT / "source-inventory.tsv"
BIB_PATH = REFERENCE_ROOT / "references.bib"
...
CURRENT_CITED_ARCHIVE_PREFIXES = (
    "public/references/70_report_adjacent_candidates/",
    "public/references/80_future_surrogates_and_pinns/",
    "public/references/90_background_archive/",
    "public/references/98_needs_review/",
    "public/references/99_duplicates_superseded/",
)
REFERENCE_NON_ARTIFACTS = {
    "public/references/AGENTS.md",
...
def tracked_reference_artifacts(repo: Path) -> set[str]:
    files = git_lines(repo, ["ls-files", "--", REFERENCE_ROOT.as_posix()])
...
def bib_keys(repo: Path) -> set[str]:
    text = (repo / BIB_PATH).read_text(encoding="utf-8")
...
def check_archive_coverage(
    ...
    for row in rows:
        ...
        if local_path and not local_path.startswith("public/references/"):
            issues.append(
                AuditIssue(
                    INVENTORY_PATH.as_posix(),
                    line,
                    "inventory-path-outside-references",
                    f"local_path must stay under public/references/ or point to a private references mirror: {local_path}",
                )
            )
        ...
        if bib_key and bib_key not in known_bib_keys:
            issues.append(
                AuditIssue(
                    ...
                    line,
                    "bib-key-missing",
                    f"bib_key is not present in {BIB_PATH.as_posix()}: {bib_key}",
                )
            )
        ...
        if filename.startswith("unknown_") and not local_path.startswith("public/references/98_needs_review/"):
            issues.append(
                AuditIssue(
                    INVENTORY_PATH.as_posix(),
                    line,
                    "unknown-outside-review",
                    f"unknown metadata files must live under public/references/98_needs_review/: {local_path}",
                )
            )
...
def check_tex_citations(known_bib_keys: set[str], known_cited_keys: set[str]) -> list[AuditIssue]:
    ...
    for key in sorted(known_cited_keys - known_bib_keys):
        issues.append(AuditIssue(BIB_PATH.as_posix(), 1, "cited-key-missing", f"cited key is missing: {key}"))
    return issues
