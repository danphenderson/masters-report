...
"""
struct StationaryStokesIC <: AbstractInitialConditionSpec
...
function StationaryStokesIC(;
    ...
    pressure_drop = pressure_drop_pa === nothing ? pressure_drop_dyn_cm2 : 10.0 * Float64(pressure_drop_pa)
    pressure_value = pressure_drop === nothing ? NaN : Float64(pressure_drop)
...
function validate(ic::StationaryStokesIC)
    ...
    ic.pressure_drop_dyn_cm2 > 0.0 || throw(ArgumentError("stationary-stokes pressure drop must be positive"))
