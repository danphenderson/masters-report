...
handling, rheology/profile sensitivity, resolved-velocity output diagnostics,
and Python CPU/MPS consistency. These checks are useful because they make the
...
experimental measurements. For the comparison cases, the archived rows still
lack several quantities that would make the verification record more
discriminating: positivity-activation counts, total projection correction,
realized CFL extrema, mass-balance defect, and backend tolerances for the
specific C23/C40 runs. The present result is therefore best described as a
verified report workflow with documented limitations, not as complete numerical
verification of every possible output.
...
C40, while the maximum section error increases from 2.35 cm/s to 9.92 cm/s.
Table~\ref{tab:t1-largest-section-errors} localizes the largest section errors
...
transient-history metadata. These differences matter because velocity error is
not solely a profile-closure error. Area mismatch can change
...
$|\pi a-A_{3D}|/A_{3D}$, nor does it decompose the velocity error into
physical-flow and current-area contributions. Without that decomposition, the
comparison should be read as a coupled velocity, flow, geometry, and modelling
...
$(a,q)=(A_{\mathrm{phys}}/\pi,Q_{\mathrm{phys}}/\pi)$. It is the
Canic-extended, radius-squared finite-volume model with
...
resolved-velocity output diagnostics, and Python CPU/MPS checks support the
reported finite-volume/time-integration data contracts. They do not yet supply
every diagnostic needed for a stronger verification claim, especially for the
specific C23/C40 comparison runs.
