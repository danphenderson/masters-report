...
The local Julia project also exposes the implemented spatial and time-integration
...
\texttt{SciMLBase} and \texttt{OrdinaryDiffEq}; adaptive \texttt{vern7} and
\texttt{vern9} policies therefore use the existing SciML backend. The
characteristic WENO3 finite-volume reconstruction is implemented in-repo and is
available as an opt-in higher-order sensitivity method. WENO5 and ENO remain
future method-development options in this report.
...
Velocity and flow differences are reported as empirical sample $L^1$, $L^2$,
and $L^3$ errors over valid section targets. With
$d_{u,j}=\bar u_{1D,j}-\bar u_{3D,j}$ and
$d_{Q,j}=Q_{1D,j}-Q_{3D,j}$, the normalized sample errors are
\begin{flalign*}
    \quad
    E_{u,p}
    &=
    \left(\frac{1}{n}\sum_j |d_{u,j}|^p\right)^{1/p},
    &
    E_{Q,p}
    &=
    \left(\frac{1}{n}\sum_j |d_{Q,j}|^p\right)^{1/p},
    &
    p&\in\{1,2,3\}, &&\\
    \quad
    E_{u,p}^{\mathrm{rel}}
    &=
    \frac{\left(\sum_j|d_{u,j}|^p\right)^{1/p}}
         {\left(\sum_j|\bar u_{3D,j}|^p\right)^{1/p}},
    &
    p&\in\{1,2,3\}. &&
\end{flalign*}
