...
\subsection{MUSCL Finite-Volume and SSPRK3 Discretization}
\label{subsec:muscl-ssprk3-discretization}

The reported comparison uses the finite-volume realization specified in
...
$Q_{1D}(z,t)=\pi q(z,t)$. Radial profile plots use the parabolic-profile
reconstruction
\[
...
That reconstruction is a profile diagnostic, not a resolved secondary-flow,
separation, skewness, or wall-traction model.
...
\subsection{Numerical Experiments, Error Metrics, and Provenance}
...
available resolved 3D velocity comparison datasets at $t=1.0$ s. The cases are
labelled C23 and C40 by stenosis severity. They are computational cross-model
...
\begin{table}[!htb]
    ...
    \begin{tabularx}{\linewidth}{@{}
        ...
        Inlet condition
            ...
            & unknown
            & Flow mismatch cannot be attributed uniquely to closure error. \\
        Outlet condition
...
$e_{u,j}=\bar u_{1D,j}-\bar u_{3D,j}$, the velocity sample metrics are
\begin{flalign*}
    \quad
    \mathrm{MAE}_u
    &=
