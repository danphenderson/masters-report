...
\begin{figure}[!htb]
    ...
    \caption{Compliant-vessel notation for the 1D reduction. The current configuration
    $\OmgBt$ is represented by a straightened averaging domain with
...
\begin{figure}[!htb]
    ...
    The panels show deterministic half-domain cutaway views of the full
    upstream XDMF/HDF5 velocity fields: case 77 has 22,691 loaded nodes and
    case 60 has 21,975 loaded nodes. Color denotes $u_z$ in cm/s, sparse
    arrows indicate local vector direction, and the transparent surface is the
    node envelope used only to frame the 3D point cloud. These are resolved
    velocity-data renderings, not the stationary-Stokes pathline proxy or a
    pressure-drop, FFR, or patient-specific clinical calibration measurement.}
