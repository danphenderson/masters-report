...
\begin{definition}[Conservative incompressible Navier--Stokes]
    ...
    \end{flalign*}
\end{definition}
\todo{ADVISOR[MAJOR]: Use one forcing symbol consistently. Earlier
\(\vfb\) is body force per unit mass and \(\rho\vfb\) per unit volume, while
this section switches to \(\vf\). Define \(\vf\equiv\vfb\) for acceleration
forcing, or replace \(\vf\) by \(\vfb\) throughout the Navier--Stokes systems.}
...
\todo{ADVISOR[MAJOR]: Add a precise PDE well-posedness citation and hypotheses
here, or soften this to background context. The Clay citation supports global
regularity context, not this local theorem-strength statement.}
...
\end{definition}
\todo{ADVISOR[MAJOR]: Clarify whether
\(\partial_z p_{\mathrm g}(A,z,t)\) in Eq.~\ref{eq:1d-momentum-aq} is the
total derivative of \(p_{\mathrm g}(A(z,t),z,t)\) or the explicit
\(z\)-partial at fixed \(A\). The flux/source split later uses both notions.}

\begin{assumption}[First-run momentum-profile and rheology closure]
    \label{ass:1d-first-run-alpha-friction}
    The default reduced numerical lane uses
    \(\alpha=1.1\), \(\nu=0.04\,\mathrm{cm}^2/\mathrm{s}\), and
    \texttt{NewtonianRheology()}. The scalar constant-viscosity specialization
    may be read through the compact Poiseuille-calibrated wall-friction
    coefficient
    \begin{flalign*}
    ...
    \begin{flalign*}
        ...
        \dot\gamma_{1D}
        &\defined
        \frac{\alpha}{\alpha-1}\frac{|Q|}{a\sqrt a}, &&\\
        \quad
