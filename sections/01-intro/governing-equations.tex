...
\begin{definition}[Wall and coupling closure axes]
    ...
    \begin{tabularx}{\textwidth}{@{}
        ...
        Wall
            ...
            pressure
            & The selected single-vessel model; no separate structural PDE is
            solved. \\
        Wall
...
\begin{definition}[Classical no-slip 1D baseline]
    ...
    and wall-law axis as the selected 1D framework, but omits the Canic
    effective-alpha variable-radius correction. The descriptor
    \texttt{canic-extended-1d} records the current extended-model member.
\end{definition}
...
\begin{definition}[Physical-to-solver map for the implemented 1D state]
    ...
    \begin{flalign*}
        ...
        &&
    \end{flalign*}
    With $K=Eh/(1-\sigma^2)$, the solver elastic flux contribution recorded
    in Appendix~\ref{app:num-current-solver-operator} is
    \begin{flalign*}
        \quad
...
\begin{remark}[Stenosis-profile and variable-radius boundary]
    ...
    corrections motivate later comparison questions, but they are not included
    in the baseline model unless a separate closure identifier and numerical
