...
\begin{figure}[!htb]
    \centering
    \captionsetup{aboveskip=2pt,belowskip=2pt}
    \includegraphics[height=0.70\textheight]{figures/static/static/rendered/stenosis-particle-trajectories.pdf}
    \caption{Analytic three-dimensional renderings of the
    \(C^\infty\) reference geometry in
    Definition~\ref{def:1d-stenosis-reference-geometry}, with
    \(s_{\max}=23\%,50\%,73\%\) stacked vertically. Here \(s_{\max}\) is the
    prescribed anatomical radius-reduction parameter in the idealized geometry.
    The overlaid curves are steady particle pathlines traced through the
    generated stationary Stokes velocity field; they are not pressure-drop,
