...
modeling hierarchy. The high-fidelity starting point is the incompressible
Navier--Stokes or generalized-Newtonian system on a three-dimensional lumen.
The report then uses cross-sectional reduction to identify the selected 1D
area-flow variables and to state the closure assumptions that must be fixed
before any reduced numerical study is interpreted.
...
applied to an incompressible continuum fluid. The derivation details and
...
Sec.~1.1, pp.~380--383]{hemodynamics}
...
\begin{figure}[!htb]
    ...
    \caption{From conservation laws to incompressible Navier--Stokes. Mass
    conservation reduces to $\Div\vu=0$ under constant density. Momentum
    balance closes through the Cauchy stress law; the Newtonian limit uses
    constant dynamic viscosity, while the generalized-Newtonian form keeps
    $\eta_{\mathrm{eff}}(\dot\gamma)$ explicit.}
    \label{fig:ns-derivation-map}
\end{figure}

Mass conservation for the constant-density incompressible model gives
\begin{flalign*}
...
\begin{definition}[Conservative incompressible Navier--Stokes]
    ...
    \begin{flalign*}
        \quad & \begin{cases*}
            ...
            =
            -\nabla p+\Divbf(2\eta\vct{D}(\vu))+\rho\vf, \\
...
\begin{definition}[Generalized-Newtonian incompressible Navier--Stokes]
    ...
    \end{flalign*}
    The velocity \(\vu\), pressure \(p\), body force \(\vf\), and constant
...
because it seperates the convective ($(\vu\cdot\nabla)\vu$) and diffusive ($\Divbf(2\nu_{\mathrm{eff}}(\dot\gamma)\vct{D}(\vu))$) contributions in the momentum equation, allowing for more straightforward numerical treatment of each term independently.
...
\begin{remark}[Global regularity context]
    ...
    Prize problem \cite[Problem statement]{ClayNavierStokesMillennium}. This is
...
Classical local well-posedness statements require a domain, boundary
conditions, forcing, initial-data space, and solution class. Under compatible
smooth data one obtains a unique strong solution on a maximal time interval.
The domain and function-space notation used in such statements is summarized in
Appendix~\ref{app:mathematical-foundations}; coordinate and energy background
is collected in Appendix~\ref{app:ns-coordinate-details}.
...
\cite[Sec.~2, pp.~253--257]{FormaggiaLamponiQuarteroni2003}
\cite[Ch.~3, Secs.~3.2 and~3.6]{KopplHelmig2023DimensionReducedModeling}
\cite[Sec.~3, pp.~24--26]{ShiLawfordHose2011}.
...
\begin{definition}[Closed 1D area-flow model]
    ...
    For an incompressible fluid with density \(\rho\), the working compliant
    single-vessel model is
    \begin{flalign*}
        \quad \frac{\partial A}{\partial t}
        + \frac{\partial Q}{\partial z}
        &= 0, \numberthis\label{eq:1d-mass-aq} && \\
        \quad \frac{\partial Q}{\partial t}
        + \frac{\partial}{\partial z}
            \left(\alpha \frac{Q^2}{A}\right)
        + \frac{A}{\rho}\frac{\partial p_{\mathrm g}(A,z,t)}{\partial z}
        &= - C_f(z,A,Q) \frac{Q}{A}.
        \numberthis\label{eq:1d-momentum-aq} &&
    \end{flalign*}
    The parameter \(\alpha\) is the momentum-flux correction factor, \(C_f\) is
    the wall-friction closure, and \(p_{\mathrm g}(A,z,t)\) is the wall-law
    gauge pressure closing the reduced system
    \cite[Sec.~2, pp.~253--255]{FormaggiaLamponiQuarteroni2003}
    \cite[Ch.~3, Sec.~3.2, pp.~40--47]{KopplHelmig2023DimensionReducedModeling}.
\end{definition}

\begin{assumption}[First-run momentum-profile and friction closure]
    \label{ass:1d-first-run-alpha-friction}
    The baseline reduced numerical lane fixes \(\alpha=1\) and uses a compact
    Poiseuille-calibrated wall-friction coefficient
    \begin{flalign*}
        \quad C_f \defined \frac{8\pi\eta}{\rho}. &&
    \end{flalign*}
    This is a numerical contract for the first controlled 1D study. It should
    not be read as a claim that the flat-profile flux convention, friction law,
    and any derived shear proxy form a complete resolved velocity-profile model.
\end{assumption}
...
\begin{definition}[Stenosis reference geometry]
    ...
    \end{flalign*}
    A convenient localized test profile is
    \begin{flalign*}
        \quad s(z) =
        \begin{cases}
            \displaystyle
            \frac{s_{\max}}{2}
            \left[
                1 + \cos\left(\frac{2\pi(z-z_c)}{\ell_s}\right)
            \right],
            & |z-z_c| \leq \ell_s/2, \\
            0, & \text{otherwise}.
        \end{cases}
...
\begin{remark}[Stenosis-profile and variable-radius boundary]
    ...
    The raised-cosine profile in Eq.~\ref{eq:1d-stenosis-profile} is compactly
    supported and \(C^1\) at the support endpoints, but it is not a smooth
    strong-form contract for all variable-radius correction terms. Canic et al.
    show that standard 1D variable-geometry models may miss stenotic pressure
    and velocity trends unless additional variable-radius terms are included
    \cite[Abstract; Secs.~1--2]{CanicEtAl2024Extended1DStenosis}. Those
    corrections motivate later comparison questions, but they are not active in
    the baseline model unless a separate closure identifier and numerical
    contract are declared.
\end{remark}

\begin{definition}[Conservative flux and source notation]
    \label{def:1d-conservative-flux-source-notation}
    The pressure term in Eq.~\ref{eq:1d-momentum-aq} can be represented through
    an elastic-potential flux plus explicit sources:
    \begin{flalign*}
        \quad \partial_t A + \partial_z Q &= 0,
        \numberthis\label{eq:1d-conservative-mass} && \\
        \quad \partial_t Q
        + \partial_z\!\left(
            \alpha\frac{Q^2}{A}+\Psi(A,z,t)
        \right)
        &=
        S_{\mathrm{geom}}(A,z,t)+S_f(A,Q,z),
    ...
    \begin{flalign*}
        ...
        \partial_A \Psi(A,z,t)
        &=
        \frac{A}{\rho}\partial_A p_{\mathrm g}(A,z,t), \\
        S_f(A,Q,z)
        &=
        -C_f(z,A,Q)\frac{Q}{A}.
        &&
    \end{flalign*}
    This notation records the balance-law objects needed by finite-volume
    schemes; it does not assert that every later implementation treats the flux
    and source terms identically
...
Under this regime, a finite-volume boundary realization constrains the incoming
...
after geometry, wall response, rheology, inlet and outlet data, numerical
resolution, output operators, and discrepancy metrics are matched.
...
\begin{figure}[!htb]
    ...
    manuscript selects a 1D reduced forward model; 3D, FSI, clinical, and
    independent-solver comparisons require separately matched evidence.}
...
\begin{table}[!htb]
    ...
    \begin{tabularx}{\textwidth}{@{}
        ...
        Classical 1D compliant-vessel models
            ...
            & Main reduced model tier under declared wall, friction, inlet, and
            outlet assumptions \\
        Stenosis-aware 1D and pressure-loss ROMs
            ...
            clinical evidence \\
        2D, 3D, and FSI models
            & Higher-fidelity local flow and wall-motion comparison
            & Velocity, pressure, wall traction, WSS, wall displacement, mesh and
            time diagnostics
            & Reference-tier context unless the same geometry, rheology, wall,
            boundary data, and outputs are compared directly \\
...
Across these categories, the recurring descriptors are stenosis geometry, wall
stiffness or compliance, density, viscosity or effective-viscosity law, inlet
waveform, outlet pressure or load, pressure-reference convention, and tap
locations. The recurring outputs are pressure drop, reduced pressure ratio,
area, flow, radius, and either a reduced wall-friction/shear proxy or a
resolved traction-based WSS quantity
\cite{ShiLawfordHose2011,FormaggiaLamponiQuarteroni2003,KopplHelmig2023DimensionReducedModeling}
\cite{CanicEtAl2024Extended1DStenosis,FormaggiaMouraNobile2007Coupling}.
