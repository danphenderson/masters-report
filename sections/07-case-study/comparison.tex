...
\begin{table}[!htb]
    \centering
    \scriptsize
    \pgfplotstableread[col sep=space]{figures/static/static/data/stenosis-comparison/production-diagnostics.dat}\TOneProductionDiagnostics
...
between the current 1D section means and the previous grid's 1D section means.
...
\begin{table}[!htb]
    ...
    in cm/s, and the adjacent column compares the current 1D section means to
    the previous grid on the same 200 section targets. The first grid has no
    adjacent predecessor.}
    \label{tab:t1-grid-sensitivity-summary}
    \resizebox{\textwidth}{!}{%
    \IfFileExists{figures/static/static/tables/stenosis-comparison/grid_sensitivity_summary.tex}{%
        \input{figures/static/static/tables/stenosis-comparison/grid_sensitivity_summary.tex}%
    }{%
        \begin{tabular}{ll}
        \toprule
        Status & Message \\
