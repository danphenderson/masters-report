\subsection{Model and Computational Methodology}
\label{sec:methodology}

This section is the implementation contract for the worked stenosis example. It
fixes the physical variables, solver-coordinate map, stenosis geometry, wall
...
\subsubsection{Physical and Solver Variables}
...
\subsubsection{Geometry and Wall Law}
...
\subsubsection[Implemented Rmax-Normalized Balance Law]{Implemented $R_{\max}$-Normalized Balance Law}
...
\begin{definition}[Closed 1D area-flow model]
    ...
    \begin{flalign*}
        \quad
        \frac{d}{dz}p_{\mathcal W,\mathrm g}(A(z,t),z,t)
        =
        \pd_A p_{\mathcal W,\mathrm g}(A,z,t) \pd_z A
        +
    ...
    \begin{flalign*}
        ...
        + \frac{A}{\rho}\frac{d}{dz}p_{\mathcal W,\mathrm g}(A(z,t),z,t)
        &= - C_f(z,A,Q) \frac{Q}{A}.
    ...
    closure, and $p_{\mathcal W,\mathrm g}(A,z,t)$ is the wall-law gauge
    pressure closing the reduced system.
\end{definition}
...
\begin{definition}[Conservative flux/source notation for the reduced wall law]
    ...
    $p_{\mathcal W,\mathrm g}$ is the wall law in
...
\subsubsection{Principal MUSCL/Rusanov/SSPRK3 Realization}
...
rows using the same cut triangles, but those rows are quarantined in this
manuscript because direct recomputation from the tracked radial data has not
been reconciled with the existing summary values. No radial-profile figure,
table, or interpretation is used as retained evidence.
...
\subsubsection{Comparison Design, Discrepancy Metrics, and Provenance}
...
sample $T=0.9995$ s. The cases are labelled C23 and C40 by stenosis severity.
The comparison is a diagnostic cross-model velocity comparison under the
declared observation operator, and the model-matching matrix below states the
evidence boundary for interpreting the retained C23/C40 section discrepancies.
...
\path{figures/static/static/data/stenosis-comparison/} and
\path{figures/static/static/tables/stenosis-comparison/}, interpreted under
Convention~\ref{conv:computed-realization-record}. The baseline regeneration
command, optional grid-sensitivity command, source-path convention, and
file-hash manifest pointer are listed in Appendix~\ref{app:code-and-ai-use}.
Scratch reruns that are not published as tracked static assets live under
\path{simulations/output/revision-*}, \path{simulations/output/3d_comparison/},
or \path{tmp/revision-evidence/}. The raw XDMF/HDF5 inputs remain local ignored
files under
\path{simulations/data/3d/canic_case3/}; the report archives the generated CSV
and PGFPlots-ready static data, not the raw fields.

Table~\ref{tab:t1-evidence-ledger} records the claim-to-evidence contract used
for the retained case-study statements. A row in this ledger is not an
additional result. It is the rule for reading the following tables and figures:
the listed asset is the source for the retained statement, and the listed limit
is part of the claim.

\begin{table}[!htb]
    \centering
    \scriptsize
    \caption{Evidence ledger for the retained C23/C40 case-study claims. The
    ledger separates source artifacts, computational scope, and interpretation
    limits so that later tables are read as bounded evidence rather than
    standalone validation claims.}
    \label{tab:t1-evidence-ledger}
    \begin{tabularx}{\textwidth}{@{}
        ...
        \textbf{Claim class}
            & \textbf{Primary asset}
            & \textbf{Computation scope}
            & \textbf{Interpretation limit} \\
        \midrule
        Output-operator check
            & \path{cross-section-operator-}\newline\path{validation.csv}
            & Synthetic tetrahedron; constant and affine axial fields; shifted
            plane cuts.
            & Verifies the implemented quadrature map on known fields, not the
            physical fidelity of the available 3D cases. \\
        Run admissibility
            & \path{production-diagnostics.dat} and
            \path{rest_state_drift.tex}
            & Native MUSCL/Rusanov/SSPRK3 runs, recorded CFL, positivity,
            subcriticality, and finite-volume bookkeeping diagnostics.
            & Establishes numerical regime and the rest-state limitation; it
            does not validate a reduced model against resolved data. \\
        Section discrepancy
            & \path{section-quadrature.dat}
            & 200 axial planes at $T=0.9995$ s for C23 and C40 under
            \texttt{CrossSectionQuadratureOperator}.
            & Descriptive final-time cross-model discrepancy with unmatched
            wall, boundary, material, and history metadata. \\
        Production-grid sensitivity
            & \path{grid-sensitivity-summary.csv}
            & Same section protocol at $N=200,400,800$ for the optional local
            C23/C40 raw fields.
            & Sensitivity evidence for the reported output, not a monotone
            convergence theorem. \\
        Excluded radial output
            & \path{radial-quadrature-C23.dat} and
            \path{radial-quadrature-C40.dat}
            & Generated secondary rows at three axial slices.
            & Quarantined from retained claims pending reducer reconciliation. \\
        \bottomrule
    \end{tabularx}
\end{table}
...
\begin{table}[!htb]
    ...
    records the upstream local data directory, and the 1D target time is matched
    to the available XDMF velocity sample.}
    \label{tab:t1-case-matching}
    \begin{tabularx}{\textwidth}{@{}llXlrrr@{}}
        \toprule
        \textbf{Label}
            & \textbf{Source ID}
        ...
        C23 & 77 & \path{77/velocity.xdmf}
            & \texttt{CrossSectionQuadratureOperator}
            & 0.9995 & 0.9994999999999453 & $5.47\times10^{-14}$ \\
...
\begin{table}[!htb]
    ...
    \begin{tabularx}{\linewidth}{@{}
        ...
        Current lumen area
            ...
            & unresolved static cut
            & The tracked section asset supports static cut-area auditing and the
            reported flow/area split, but not a deformed-wall area attribution. \\
        Wall model
        ...
        Time / transient history
            & Evaluated at $T=0.9995$ s from a start-up transient.
            & XDMF time is $0.9995$ s; earlier history is not recorded.
            & timestamp matched
            & The final timestamps are matched, but phase/history matching is not
            established. \\
        Numerical resolution
            & Principal $N=400$ finite-volume cells; optional output
            sensitivity at $N=200,400,800$.
            & Node-centered velocity fields with loaded node counts reported.
            & sensitivity checked
            & The grid record tests production-output sensitivity. It is not a
            convergence proof because the same unmatched 3D metadata and model
            differences remain present at every grid. \\
        Velocity observable
...
Table~\ref{tab:t1-parameter-matching} is the compact notation-and-parameter
record for the principal comparison run. It consolidates the selected model
descriptor, wall and rheology closures, physical constants, discretization,
boundary convention, and observation targets before the discrepancy metrics are
introduced.
...
\begin{table}[!htb]
    ...
    $t=0.9995$ comparison. The generated comparison records characteristic-speed
    ...
    \begin{tabularx}{\textwidth}{@{}p{0.22\textwidth}X@{}}
        ...
        \midrule
        Package & \texttt{StenosisHemodynamics}. \\
        ...
        $p_{\mathrm{ext}}\equiv0$. \\
        Discretization & Principal $N=400$, $L=6$ cm, MUSCL finite volume with
        minmod limiter, native SSPRK3 time integration; optional
        grid-sensitivity rows repeat the output comparison at $N=200,400,800$. \\
        Time controls & $T=0.9995$ s from a start-up transient, timestep cap
        $10^{-5}$ s, CFL cap 0.45. \\
        ...
        cm. \\
        Radial targets & Slices $z=1.951$, 2.451, and 2.951 cm are generated
        as secondary rows, but radial-profile plots and interpretations are
        quarantined from the retained evidence. \\
        ...
        0.0150753769, and 0.0301507538 cm; production-grid sensitivity is stored
        to \path{grid-sensitivity-summary.csv}. \\
        \bottomrule
    \end{tabularx}
\end{table}
