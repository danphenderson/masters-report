        ...
        if bib_key and bib_key not in known_bib_keys:
            issues.append(
                AuditIssue(
                    ...
                    line,
                    "bib-key-missing",
                    f"bib_key is not present in references/references.bib: {bib_key}",
                )
            )
...
def check_tex_citations(known_bib_keys: set[str], known_cited_keys: set[str]) -> list[AuditIssue]:
    ...
    for key in sorted(known_cited_keys - known_bib_keys):
        issues.append(AuditIssue("references/references.bib", 1, "cited-key-missing", f"cited key is missing: {key}"))
    return issues
