...
The package also provides a separate reconstruction and diagnostic comparison
workflow for Canic et al.\ Section~4.1. This workflow is distinct from the main
23\%/40\% manuscript comparator above: it reconstructs
local 1D outputs, compares section-mean velocity against restored upstream 3D
bundles, emits gauge-dependent pressure diagnostics, and writes parameter and
upstream-provenance comparisons. The 50\% upstream bundle is included by the
same per-case final-time rule; it remains source-artifact comparison output
...
packages/stenotic-hemodynamics/bin/stenotic-hemodynamics canic-replication section41 \
  ...
  --overwrite
\end{lstlisting}
The workflow emits provenance, parameter-comparison, comparison-summary,
section-comparison, radial-velocity, and compact 3D-diagnostic files. Its checks
intentionally preserve source inconsistencies, including the PDF Table~1 Young
...
Table~\ref{tab:canic-section41-parameter-audit} lists the non-identical source
