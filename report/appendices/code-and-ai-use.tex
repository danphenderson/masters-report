\section{Software, Source Artifacts, and AI-Assisted Work Record}
...
\end{lstlisting}
This appendix therefore records local source-artifact traceability rather than
claiming a public archive. No public repository URL, immutable archive URL, or DOI is
...
\paragraph*{Canic Section 4.1 source-artifact workflow.}
The package also provides a separate source-artifact reconstruction and
diagnostic comparison workflow for Canic et al.\ Section~4.1. This workflow is
distinct from the retained 23\%/40\% manuscript comparator above: it reconstructs
local 1D outputs, compares section-mean velocity against restored upstream 3D
bundles, emits gauge-dependent pressure diagnostics, writes a parameter audit,
and records upstream provenance. The 50\% upstream bundle is included by the
same per-case final-time rule; it remains source-artifact comparison evidence
rather than replication evidence unless parameter, pressure-gauge, and
observable-tolerance criteria are also satisfied. The upstream MATLAB code is treated as
...
packages/stenotic-hemodynamics/bin/stenotic-hemodynamics canic-replication section41 \
  ...
  --overwrite
\end{lstlisting}
The workflow emits provenance, parameter-audit, comparison-summary,
...
snapshot time.
Table~\ref{tab:canic-section41-summary} summarizes the stored
section-averaged velocity and pressure-diagnostic rows, and
Table~\ref{tab:canic-section41-parameter-audit} records the non-identical source
conditions that bound any Section~4.1 reconstruction or comparison claim. Missing raw
inputs are non-fatal and print:
\begin{lstlisting}[basicstyle=\ttfamily\footnotesize,breaklines=true]
canic_replication_status,skipped_missing_data
\end{lstlisting}
Derived outputs from prior reviewed runs are stored in
\path{report/assets/data/canic-replication/} and
\path{report/assets/tables/canic-replication/}; the raw XDMF/HDF5 bundles
remain ignored local inputs. Those stored table fragments predate the current
time-alignment and pressure-gauge policy and are not input as current evidence
in this thesis revision. Corrected numerical rows require a separate
...
\begin{table}[!htb]
    ...
    differ outside the \(10^{-6}\,\mathrm{s}\) tolerance. Pressure is withheld
    as cross-model error evidence until a common gauge is implemented.}
    ...
    \begin{tabular}{@{}lllll@{}}
        ...
        \midrule
        23\% & \(0.9995\,\mathrm{s}\) & \(0.9995\,\mathrm{s}\) & time-aligned source-artifact comparison & non-evidentiary diagnostic \\
        40\% & \(0.9995\,\mathrm{s}\) & \(0.9995\,\mathrm{s}\) & time-aligned source-artifact comparison & non-evidentiary diagnostic \\
        50\% & \(1.4995\,\mathrm{s}\) & \(1.4995\,\mathrm{s}\) & time-aligned source-artifact comparison & non-evidentiary diagnostic \\
        \bottomrule
    \end{tabular}%
    }
\end{table}
...
\begin{table}[!htb]
    \centering
    \scriptsize
    \caption{Current Canic Section~4.1 parameter and provenance audit boundary.
    ...
    \begin{tabular}{@{}lll@{}}
        ...
        Pressure gauge & no common 1D--3D pressure-gauge operator is implemented here & absolute pressure error withheld \\
