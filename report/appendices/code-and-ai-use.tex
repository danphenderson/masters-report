...
point, and command-line interfaces are:
...
This advisor-review source record is local rather than a public archived
release. It corresponds to the manuscript source rooted at
\path{report/final-report.tex}, the local reproducibility manifest below, the
generated manuscript assets under \path{report/assets/}, and the retained Canic
case3 source-data bundle under
\path{public/var/data/simulations/canic_case3/}. The local release manifest is:
\begin{lstlisting}[basicstyle=\ttfamily\footnotesize,breaklines=true]
public/reproducibility/release-manifest.json
\end{lstlisting}
That manifest records \texttt{release\_tag=not\_declared\_for\_this\_local\_copy},
\texttt{public\_repository\_url=not\_declared}, and
\texttt{archival\_doi=not\_declared}. This appendix therefore describes a
locally reproducible advisor-review source copy without asserting a public
repository URL, immutable archive URL, or DOI.

\paragraph*{Archive contents.}
The local source record contains the tracked manuscript source, package source,
dependency manifests or lockfiles, accepted report assets, and reproducibility
records needed to rebuild this copy. It excludes local virtual environments,
Julia depots, caches, editor state, and other machine-specific rebuild products
such as \path{.venv/}; those environments are recreated from the recorded
manifests and commands. The retained Canic case3 XDMF/HDF5 bundle is tracked in
this source package as source-data input with checksum provenance and is not
duplicated as a report asset. Any later public release can add a tag, archive
URL, DOI, or separate data-archive record without changing this local
...
The repository README states the intended portable execution environment as a macOS
...
public/docs/markdown/manuscript-claim-register.md
\end{lstlisting}
...
packages/stenotic-hemodynamics/bin/stenotic-hemodynamics verify rest \
  ...
  --severities 22.555555555555554,40 \
...
Table~\ref{tab:rest-method-sensitivity} were generated as auxiliary experiments
and then rendered into report CSV/\TeX{} fragments:
\begin{lstlisting}[basicstyle=\ttfamily\footnotesize,breaklines=true]
pipenv run ops-experiment \
  --run-id additional-rest-fv-first-order -- \
  verify rest --space fv-first-order \
  --severities 22.555555555555554,40,50 \
  --nxs 400 \
...
pipenv run ops-experiment \
  --run-id additional-rest-fv-wb-geometry-rest -- \
  verify rest --space fv-wb-geometry-rest \
  --severities 22.555555555555554,40,50 \
  --nxs 400 \
  --elapsed-times 0,0.001,0.01,0.1 \
  --output-dir tmp/simulations/output/additional-evidence/rest-fv-wb-geometry-rest \
  --overwrite
...
\end{lstlisting}
The well-balanced row is the promoted geometry-rest-preserving finite-volume
method used by the refreshed comparison record. The Lax--Wendroff rows are
retained as diagnostic-method context rather than selected comparison evidence.
The resolved-velocity comparison uses the C23 (22.56\%) and 40\% stenosis-severity
descriptions in the main text. These correspond to local source IDs 77 and 60,
respectively. The retained raw 3D fields are tracked source-data inputs rather
than report assets; the detailed input root and upstream pointer are recorded
below.

\paragraph*{Resolved-comparison reconstruction commands.}
Using the tracked raw 3D input root, the generated comparison data are
regenerated with:
\begin{lstlisting}[basicstyle=\ttfamily\footnotesize,breaklines=true]
...
pipenv run ops-render-case-study-evidence-tables
\end{lstlisting}
A checkout containing the tracked Canic case3 XDMF/HDF5 inputs can rebuild the
manuscript, regenerate the retained raw-data-dependent comparison assets, and
verify absent raw resolved-3D inputs as an expected skip by pointing the command
at a deliberately missing root. The remaining timing limitation is
...
et al.\ Section~4.1. It is distinct from the main C23 (22.56\%)/40\% manuscript
comparator above: it reconstructs local 1D outputs, compares section-mean
velocity against tracked upstream 3D bundles, emits outlet-gauge pressure
...
\path{report/assets/tables/canic-replication/}; the retained raw XDMF/HDF5
bundles are tracked under \path{public/var/data/simulations/canic_case3/} with
checksums recorded in the provenance JSON. The generated Canic fragments used in
this revision were regenerated after the imported-time and outlet-gauge pressure
policy landed, so they are current reproducibility diagnostic assets.

\begin{table}[!htb]
    \centering
    \scriptsize
    \caption{Current Canic Section~4.1 reproducibility comparison boundary.
    Imported times are read from the tracked XDMF bundles, and the default
    ...
    \begin{tabular}{@{}lllll@{}}
        ...
        \midrule
        C23 (22.56\%) & \(0.9995\,\mathrm{s}\) & \(0.9995\,\mathrm{s}\) & time-aligned reproducibility comparison & outlet-gauged diagnostic \\
...
\begin{table}[!htb]
    ...
    \begin{tabular}{@{}lll@{}}
        ...
        Raw bundles & tracked XDMF/HDF5 source-data inputs live under \path{public/var/data/simulations/canic_case3/} with checksum provenance & regeneration input, not report asset \\
        \bottomrule
    \end{tabular}%
    }
\end{table}
...
the generated manuscript wall profile, fixed-point history, and summary table:
\begin{lstlisting}[basicstyle=\ttfamily\footnotesize,breaklines=true]
...
checks are independent of the raw HDF5/XDMF velocity fields; they do not
validate the available resolved meshes.

Using the tracked raw 3D input root, the reported grid sensitivity is:
\begin{lstlisting}[basicstyle=\ttfamily\footnotesize,breaklines=true]
packages/stenotic-hemodynamics/bin/stenotic-hemodynamics compare-3d \
  --data-root public/var/data/simulations/canic_case3 \
...
C23 (22.56\%) and 40\% severity section means and physical flows, with $N=1600$ kept as
...
Section~\ref{sec:resolved-3d-comparison} are tracked in this source package for
the retained Canic case3 labels. The input root is:
\begin{lstlisting}[basicstyle=\ttfamily\footnotesize,breaklines=true]
public/var/data/simulations/canic_case3/
\end{lstlisting}
The retained directory was sourced from this upstream repository, commit, and
subtree:
\begin{lstlisting}[basicstyle=\ttfamily\footnotesize,breaklines=true]
...
inventory. It records the build command, tracked resolved-input convention,
upstream 3D input pointer, comparison reconstruction commands, and the absence
of a declared public repository URL or archival DOI. The report-facing artifact
inventory and command matrix are maintained in
\path{public/docs/markdown/resolved3d-workflows.md}.
