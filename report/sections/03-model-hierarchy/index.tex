...
rules, not merely mesh geometry. A 0D model stores lumped pressure, volume,
...
operators.
Two closure axes should be kept separate from this dimensional label. The wall
closure records whether the vessel is fixed, reduced through a pressure--area
law, or solved as a structural FSI state. The coupling closure records whether a
single vessel is isolated or connected to a 0D, 1D, or 3D tier through pressure,
flow, traction, impedance, or characteristic data. Both axes are independent of
the rheology law and both affect what a comparison can claim
\cite{FormaggiaEtAl2003OneDimensionalBloodFlow,QuarteroniVeneziani2003GeometricalMultiscale,FormaggiaEtAl2007FSICoupling}.
...
Resolved FSI changes the wall state itself, not merely the list of reported
diagnostics. A compliant 1D wall law remains a reduced pressure--area closure;
it is not a substitute for wall displacement, wall velocity, structural stress,
and interface-work variables carried by a resolved FSI solve.
...
closure or observation operator supplies them
...
Those closures are distinct evidence dimensions: the state variables determine
native outputs; the wall law controls area response and wave speed; profile,
friction, and rheology choices control convective and dissipative terms; the
stenosis treatment determines how narrowed geometry enters the balance law; and
boundary and numerical choices determine the admissible comparison record.
Tube-flow and hemorheology evidence are therefore used only as closure support,
not as a license to report resolved local stresses from a reduced state
\cite{PriesEtAl1992BloodViscosityTubeFlow,GaldiEtAl2008HemodynamicalFlows,KumarEtAl2019CarreauYasudaHemodynamics,LiuEtAl2021ICASRheology}.
...
compared by target observable and closure specification, not collected under a
single ``1D'' label.
...
\cite{ShiEtAl2011BloodFlowModelReview,KopplHelmig2023DimensionReducedModeling,Wang2014BloodFlowNetworks}.
Their state variables are not local wall or velocity fields. In a single-vessel
calculation, these models most often enter through inlet or outlet closure
rather than as the main spatial solver. They do not natively contain local
stenosis geometry, an axial pressure-drop field, a velocity profile,
recirculation, resolved WSS, or wall traction; each of those quantities would
require an added spatial model or observation map.
...
Surrogate and scientific-ML models add a second axis to the hierarchy, but they
do not remove the state, closure, and observable questions. A PINN constrains an
approximate solution or inverse map with residual, boundary, data, and
regularization terms, while hemodynamics-specific reviews ask how that framework
translates to vascular flow settings
...
\cite{LuEtAl2021DeepONet,LiEtAl2021FNO}. Recent stenosis or blood-flow learning
examples are therefore read only through their declared training target,
input functions or parameters, output observable, geometry family, and
evaluation domain
\cite{CsalaEtAl2025PCNDE,VelikorodnyEtAl2025DeepOperatorStenosed}.
...
steps. The first table summarizes retained state, reduction, closure, and data
requirements. The second summarizes observables, evidence role, and
interpretation limits. The split keeps the reading frame organized around
state and observation rather than around individual papers. Read each row as a
compact specification: what the tier solves natively, what it closes, what it
can report directly, and which evidence role its outputs can support.

\begin{table}[!htb]
    \centering
    \scriptsize
    \caption{State, reduction, closure, and data requirements for cardiovascular
    model tiers.}
...
\begin{table}[!htb]
    ...
    \begin{tabularx}{\textwidth}{@{}
        ...
        Learned surrogate
            ...
            bounded by the data-generating assumptions. \\
        \bottomrule
    \end{tabularx}
\end{table}
...
Blanco's 1D--3D FFR study is read here as an FFR-specific matched-observable
...
an observation operator from 1D area--flow variables to the chosen velocity, flow, or
pressure quantity.
\end{stenosisillustration}

The next section makes those closure choices, boundary data, and observation
rules explicit before Section~\ref{sec:numerical-methods} turns to their
discrete realization and evidence standards.
