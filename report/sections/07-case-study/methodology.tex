...
The model tier, state variables, closure choices, boundary approximation,
finite-volume operator, observation rule, and matching limits are stated before
any result is interpreted. Dense operator formulas, secondary numerical
surfaces, and source-artifact reconstruction commands remain in the appendices
unless they are needed to understand a main-body claim. The paragraphs below
therefore name mathematical roles first; implementation labels appear only as
traceability tags for reconstructing the selected runs. The stack map in
...
\begin{table}[!htb]
    \centering
    \scriptsize
    \caption{Case-study specification for the 23\% and 40\%
    stenosis-severity numerical comparison. The table states how the worked
    ...
    \begin{tabularx}{\textwidth}{@{}
        ...
        Case-study purpose
            ...
            velocity fields.
            & The chapter illustrates verification, equilibrium-preservation testing, and
        ...
        Observation operator
            ...
            $T=0.9995$ s for the 23\% and 40\% stenosis cases.
            & The comparison supports a claim only for these common section
            observables. Radial-profile diagnostic rows are supplemental
            diagnostic artifacts, not localization evidence. A
            separate Section~4.1 source-artifact reconstruction and comparison
            workflow is documented in Appendix~\ref{app:code-and-ai-use}. \\
        Numerical evidence
...
\begin{remark}[Stenosis-profile and variable-radius boundary]
    ...
    parabolic-profile baseline (canonical report label
    \texttt{classical-parabolic-1d}; historical implementation label
...
\begin{definition}[Classical parabolic-profile 1D baseline]
    ...
    baseline, with canonical report label
    \texttt{classical-parabolic-1d}. Older manifests and generated source
    artifacts may retain the historical label \texttt{classical-1d-no-slip}
    as provenance. The model keeps the same reduced state variables and selected
    ...
    $R_{\max}$-normalized Canic-derived extended member is labeled
    \texttt{canic-extended-1d} in implementation manifests.
\end{definition}
...
for the selected numerical template in Section~\ref{subsec:numerical-contract-stack}.
...
an exact executable copy of every source-paper parameter and normalization in
...
Section~\ref{subsec:deformed-coordinate-comparison}. The raw-input policy,
target-time command contract, and table-fragment regeneration command are
recorded in Appendix~\ref{app:code-and-ai-use}. The broader Canic
Section~4.1 source-artifact comparison uses the same pressure and displacement
loader surfaces for the restored 23\%, 40\%, and 50\% upstream bundles, with
the local 1D target time taken from the imported XDMF time for each case. The
output appears as a separate source-artifact diagnostic table, not as the primary two-case
localization claim; pressure discrepancies in that workflow use the common
Section~4.1 outlet-gauge diagnostic and remain separate from the primary
velocity-observation claim.
...
sample $T=0.9995$ s. The reported cases are described by their 23\% and 40\%
stenosis severities. The comparison is a diagnostic cross-model velocity
comparison under the specified observation operator. Source identifiers, input
