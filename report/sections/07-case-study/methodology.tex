...
The manuscript tables and figures use tracked static report assets in the
\path{stenosis-comparison/} subdirectories under \path{report/assets/data/} and
\path{report/assets/tables/}, interpreted under
Convention~\ref{conv:computed-realization-record}. The regeneration commands,
source-path convention, and file-hash manifest pointer are listed in
Appendix~\ref{app:code-and-ai-use}. Scratch reruns that are not published as
tracked static assets live in ignored simulation or review-output paths. The raw
XDMF/HDF5 inputs remain local ignored files under the documented
\path{public/var/data/simulations/} input root. The report archives generated CSV
