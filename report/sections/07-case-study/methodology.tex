...
sample $T=0.9995$ s. The case labels C23 and C40 denote the retained 23\% and
40\% stenosis-severity cases, respectively. The comparison is a diagnostic
cross-model velocity comparison under the declared observation operator.
Table~\ref{tab:t1-case-study-contract} gives the compact contract for reading
the retained two-case evidence. Regeneration commands, local source identifiers,
raw-input policy, and static asset ownership are recorded in
...
\begin{table}[!htb]
    ...
    \begin{tabularx}{\textwidth}{@{}
        ...
        Case-study purpose
            ...
            cross-model velocity comparison; it is not a clinical validation
            study or a pressure-ratio study. \\
        Case labels and final-time match
            & C23 and C40 are short labels for the retained 23\% and 40\%
            stenosis-severity cases. Both are compared at $T=0.9995$ s, with
            recorded 1D--3D final-time offsets of $5.47\times10^{-14}$ s.
            & The final timestamp is matched. Earlier transient-history
            matching is not carried by the retained record and would need to be
            established before making a validation-level claim. \\
        ...
        Observation operator
            ...
            $\bar u_{1D}=q/a$ on 200 axial sections.
            & The comparison is meaningful only for these common section
            observables. Radial-profile outputs are excluded from retained
            claims. \\
        Numerical admissibility gates
            ...
            bookkeeping.
            & These records support run admissibility and implementation
            interpretation. They do not convert the two-case comparison into
            model validation. \\
        Matching limits
            & The analytic reference profile is shared and the static cut-area
            audit is reported, but current/deformed 3D geometry handling,
            wall motion, boundary histories, material parameters, and earlier
            transient histories remain unmatched or unpersisted.
        ...
        Retained outputs
            ...
            cut-area audit, axial flow behavior, flow/area bookkeeping,
            section-mean velocity discrepancy summary, production-grid
            sensitivity, and largest section discrepancies.
        ...
        Excluded radial/profile outputs
            & Secondary radial-profile rows exist for selected axial slices but
            are not used for any figure, table, discrepancy metric, or
            localization claim.
            & The radial reducer remains quarantined pending reconciliation
