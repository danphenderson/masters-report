\subsection{Case-Study Role and Model Specification}
\label{sec:methodology}

The worked stenosis example begins by fixing the model tier, state variables,
closure choices, boundary approximation, finite-volume method, observation map,
and comparison limits before any result is interpreted. Detailed operator
formulas, secondary numerical checks, and reconstruction commands remain in the
appendices unless they are needed for a main-text claim.
Table~\ref{tab:t1-case-study-contract} gives the compact case-study
specification used by the subsections below.

\begin{center}
\fbox{\begin{minipage}{0.90\linewidth}
\small
\textbf{Solver-coordinate convention.} In the comparison outputs, the stored
solver coordinate \texttt{A} is $a=R^2$, not physical circular area. The physical
...
\begin{table}[!htb]
    \centering
    \scriptsize
    \caption{Case-study specification for the C23 (22.56\%) and 40\%
    stenosis-severity numerical comparison.}
    ...
    \begin{tabularx}{\textwidth}{@{}
        ...
        \toprule
        \textbf{Specification item}
            & \textbf{Main-body statement}
            & \textbf{Interpretation boundary} \\
        \midrule
        Purpose
            & Controlled idealized stenosis example for one selected 1D
            area--flow model and two resolved velocity fields.
            & Supports a methodological velocity comparison, not
        ...
        Model and closures
            ...
            $\nu=0.04$ cm$^2$/s, parabolic profile closure, $N=400$
            geometry-rest-balanced MUSCL finite-volume cells, Rusanov-type
            flux, and SSPRK3 time integration.
            & Other variants provide secondary sensitivity context. \\
        Observation operator
            & The 3D velocity field is reduced by the specified
            cross-section quadrature operator; the 1D state is observed as
            physical flow $Q_{1D}=\pi q$ and mean velocity
            $\bar u_{1D}=q/a$ on 200 axial sections. A second
            comparison applies the resolved displacement field before forming
            the same plane cuts. Both coordinate modes use
            $T=0.9995$ s for the C23 (22.56\%) and 40\% stenosis cases.
            & The comparison supports claims for these common section
            observables. Radial-profile rows remain supplemental diagnostics;
            Appendix~\ref{app:code-and-ai-use} records data availability and
            reconstruction details. \\
        Numerical evidence
            & The main text reports manufactured-solution behavior,
            output-operator verification, geometry-rest preservation, subcritical
        ...
        Comparison limits
            ...
            quasi-static membrane-FSI comparator is also included for the
            radial wall displacement solve.
            & The resolved data provide final-time velocity and displacement
            fields without matched boundary histories or full transient wall
            history. \\
        \bottomrule
    \end{tabularx}
\end{table}

\subsubsection{Model Specification and Native Variables}
...
\begin{definition}[Section-averaged fields and observed pressure]
    ...
    \end{flalign*}
    Here $p_{3D,\mathrm g}$ denotes imported continuum pressure in its available
    gauge convention. The observed representative $\bar p_{3D,\mathrm g}$ is the
    section-average observation of that field, not a pointwise pressure value. It
    is distinct from the reduced wall-law pressure variable introduced below, and
    no absolute 1D--3D pressure-error claim follows unless a common pressure
    gauge is defined and applied.
\end{definition}
...
\begin{remark}[Stenosis-profile and variable-radius boundary]
    ...
    \texttt{canic-extended-1d}; the classical parabolic-profile baseline
    \texttt{classical-parabolic-1d} retains the same selected wall law but
...
\begin{definition}[Classical parabolic-profile 1D baseline]
    ...
    baseline, with canonical report label \texttt{classical-parabolic-1d}.
    The model keeps the same reduced state variables and selected
    Canic-derived pressure--area wall law as the extended 1D framework, but
    omits the Canic effective-alpha variable-radius correction. The
    $R_{\max}$-normalized Canic-derived extended member is the canonical
    comparison label \texttt{canic-extended-1d}.
\end{definition}
...
\begin{definition}[Selected Canic--Koiter pressure--area wall law]
    ...
    wall/geometry source below. The package distinguishes the
    $R_{\max}$-normalized evolution pressure from a local-denominator pressure
    diagnostic $p_{\mathrm{loc},\mathrm g}^{\mathrm{diag}}$; benchmark pressure
    extrema and pressure-diagnostic bars refer to that diagnostic convention, not
    to $p_{\mathcal W,\mathrm g}^{\mathrm{evol}}$. The 1D model identifies
...
\subsubsection{Reduced Balance Law and Rest-State Equilibrium}
...
state the specification needed to interpret the evidence.
...
the geometry-rest equilibrium-preservation test in
...
This proposition supplies the model-side equilibrium target in
Definition~\ref{def:well-balanced-checklist} for the selected numerical template
in Section~\ref{subsec:numerical-contract-stack};
Section~\ref{subsec:rest-state-drift} then evaluates the discrete residual and
drift generated by the selected numerical operator.

Continuous compatibility does not imply discrete preservation. The selected
comparison method therefore uses the geometry-rest-balanced finite-volume
variant specified in Appendix~\ref{app:num-current-solver-operator}; the
historical MUSCL/Rusanov row is retained only as nonselected numerical-methods
context.

\subsubsection{Relation to the Source Model}
\label{subsec:source-to-implementation-variant}

The selected realization is a documented Canic-derived extended model. It uses
the phrase \emph{$R_{\max}$-normalized Canic-derived extended model} when the
distinction from \cite{CanicEtAl2024Extended1DStenosis} matters. The main
differences are the $R_{\max}^{-2}$ elastic normalization, the parabolic
principal velocity-profile closure, the fixed-area characteristic boundary
approximation, and the locally frozen-viscosity treatment used for the
diagnostic $p_2$ derivative. Appendix~\ref{app:num-source-implementation-map}
contains the detailed source-model comparison. The diagnostic extended
...
\begin{definition}[Fixed-area characteristic approximation]
\label{def:fixed-area-characteristic-contract}
At each boundary call, the selected boundary approximation sets the ghost geometry to the
...
sensitivity can appear in the reported numerical discrepancies.
The approximation error enters at the boundary-state operator before
$\Psi_{\Delta t,h,\lambda_h}$, so boundary sensitivity and the geometry-rest
preservation check are reported as part of the declared boundary approximation
rather than attributed to the continuous model alone.
\end{definition}

This is therefore an implemented boundary approximation, not an exact
invariant derivation for the full variable-radius balance law.

\subsubsection{Principal Finite-Volume Method}
...
$N=200,400,800,1600,3200$ cells, retaining $N=1600$ as the intermediate
refinement and $N=3200$ as the finest reported grid, and is interpreted only
as a production-grid sensitivity check. The resolved-data comparison target time is
$0.9995\,\mathrm{s}$, matching the imported final-time samples for the C23
(22.56\%) and 40\% cases; it is distinct from the $t=1.0\,\mathrm{s}$ zero-forcing rest-state
diagnostic in Section~\ref{subsec:rest-state-drift}. The boundary-state
approximation is the fixed-area characteristic
approximation defined in
Appendix~\ref{app:num-boundary-state-realization}. Other finite-volume, modal-DG,
time-stepper, and ODE-integration variants are secondary sensitivity context
summarized in Table~\ref{tab:implemented-discretization-surface};
they are secondary to the principal two-case comparison method.
...
rows using the same cut triangles, and diagnostics classify
same-cut area and flow closure. Those rows are treated as secondary output, and
no radial-profile figure or localization claim is used until same-cut
radial-profile results are regenerated and accepted.
...
displacement before forming the plane cuts. The reference-coordinate and
deformed-coordinate section comparisons are reported in
Sections~\ref{subsec:section-mean-velocity-discrepancies}
and~\ref{subsec:deformed-coordinate-comparison}, respectively.
Appendix~\ref{app:code-and-ai-use} records data availability and reconstruction
details for the resolved inputs and supplementary Section~4.1 comparison.
...
\begin{flalign*}
    ...
    q(z_j,t)/a(z_j,t). &&
\end{flalign*}
Radial-profile rows remain supplemental to the reported comparison metrics.
...
available resolved 3D velocity datasets at the matched final-time sample
$T=0.9995$ s. The reported cases are described by their C23 (22.56\%) and 40\% stenosis
severities. The comparison is a diagnostic cross-model velocity comparison
under the specified observation rule. Data availability, reconstruction
details, and static asset ownership are kept in
...
$d_{Q,j}=Q_{1D,j}-Q_{3D,j}$, the section metrics are the signed mean
...
The metric-limit interpretation itself is reported in
Section~\ref{subsec:metric-interpretive-limits}.
