...
\begin{table}[!htb]
    ...
    \begin{tabularx}{\textwidth}{@{}
        ...
        Matching limits
            ...
            check is reported. The package now includes a quasi-static
            membrane-FSI validation workflow that can solve a radial wall
            displacement and can apply resolved displacement fields before
            plane cuts, but those outputs are not yet promoted into the
            retained C23/C40 comparison tables.
            & Velocity differences cannot be attributed uniquely to a specific
            closure, wall, boundary, or geometry mechanism until the FSI
            workflow outputs, material constants, boundary histories, and
            transient histories are retained as comparison evidence. \\
        Retained outputs
...
displacement before forming the plane cuts. The retained C23/C40 tables in this
chapter still use the earlier reference-coordinate velocity-comparison assets;
the deformed-coordinate path is therefore an implemented comparison route, not
yet a retained result.
