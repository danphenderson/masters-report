...
should be used for the already retained final-time plane cuts. The current
package can now load pressure and displacement companions and can form
deformed-coordinate plane cuts when that mode is requested, but those outputs
have not been regenerated and promoted into the retained C23/C40 comparison
assets. The 3D wall model, wall-motion history, inlet/outlet histories, material
parameters, and prior transient history are not carried by the retained
comparison record; these would need to be established before making a stronger
predictive comparison claim.

The comparison gates are therefore deliberately limited: the final timestamp is
matched, the static section area is checked, the retained dynamic or deformed
3D geometry state is unresolved, the resolved flow is not sectionwise constant, and the
wall, boundary, material, and transient-history metadata needed for a matched
reference comparison are not persisted in the retained comparison record. No
pressure-drop, pressure-ratio, or FFR-style result is retained for this 1D--3D
comparison because compatible pressure observations and extraction conventions
are not declared for the paired data.
...
\begin{table}[!htb]
    ...
    \begin{tabular}{@{}lrrrrllll@{}}
        ...
        \midrule
        C23 & 22691 & 110420 & 0.9995 & $5.47\times10^{-14}$ & node-centered & node-centered & loadable; not applied to retained cuts & $\nu,\rho$, BCs, wall law, transient history \\
        C40 & 21975 & 106547 & 0.9995 & $5.47\times10^{-14}$ & node-centered & node-centered & loadable; not applied to retained cuts & $\nu,\rho$, BCs, wall law, transient history \\
        \bottomrule
    \end{tabular}%
    }
\end{table}
...
themselves establish a matched-reference comparison. No inlet, throat, and outlet
