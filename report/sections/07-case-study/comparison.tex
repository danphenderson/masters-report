...
velocity and physical-flow observables. The zero-input rest-state test in
Section~\ref{sec:numerical-verification} remains required context because it
uses the same principal finite-volume operator as the comparison runs.
...
C40, each with an XDMF velocity sample at $0.9995$ s. The regenerated 1D
comparison runs now stop at the same target time; the recorded cross-model time
offset is $5.47\times10^{-14}$ s for both cases. This removes the previous
sample-time mismatch, but the current/deformed 3D geometry status remains
...
\begin{figure}[!htb]
    ...
    \caption{Final-time resolved velocity samples used by the C23/C40
    comparison. The panels show node-centered $u_z$ values from the tracked
    resolved-velocity CSV assets, with the corresponding node-envelope geometry
    shown only as visual context. This figure identifies the resolved fields
    being sampled; the quantitative comparison still enters through the
    declared plane-quadrature operator below.}
...
\begin{table}[!htb]
    ...
    \caption{Production diagnostics from the actual C23/C40 1D comparison runs
    completed at $T=0.9995$ s. The values are written to the tracked
    \texttt{production-diagnostics.dat} report asset.}
...
\begin{table}[!htb]
    ...
    matched $0.9995$ s final-time sample. These are single-realization
...
\begin{table}[!htb]
    ...
    \IfFileExists{report/assets/tables/stenosis-comparison/grid_sensitivity_summary.tex}{%
        ...
        Status & Message \\
        \midrule
        missing & Run \texttt{compare-3d --nxs 200,400,800} to populate the table. \\
        \bottomrule
        \end{tabular}%
    }%
    }
