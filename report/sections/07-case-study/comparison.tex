...
This section reports the retained resolved-3D comparison: the 23\% and 40\%
...
commands, and the separate Canic et al.\ Section~4.1 source-artifact workflow,
which remains supplemental to this chapter's two-case comparator.
...
as cross-model pressure errors. The separate Canic Section~4.1 workflow uses
outlet-gauged pressure diagnostics only, as recorded in
Appendix~\ref{app:code-and-ai-use}; those rows remain source-artifact
diagnostics interpreted under Section~\ref{subsec:metric-interpretive-limits}
rather than primary evidence in the two-case comparison.
...
solves; it informs wall-motion scale under the declared comparator.
...
The retained comparisons are diagnostic: they evaluate section-mean velocity
and quasi-static wall-motion scale under the declared operators. Their
...
Section~\ref{subsec:verification-scope-convergence-stability}.
Appendix~\ref{app:code-and-ai-use} records the raw-input policy and separate
Section~4.1 source-artifact workflow. Because the resolved inputs lack matched
