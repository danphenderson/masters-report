...
gauge is applied. No such common gauge is established for the reported 1D--3D
comparison, so absolute pressure discrepancies are not interpreted here as
cross-model pressure errors. Pressure values carried by imported fields or
supplemental Canic tables are diagnostics unless the workflow records a shared
...
derived velocity, gauge-dependent pressure-diagnostic, radial-postprocessing,
and 3D-diagnostic assets using the imported final time for each restored case,
but it still does not regenerate the article's raw transient 3D FSI solution.
Absolute cross-model pressure error is withheld unless a common pressure gauge
is implemented. This
