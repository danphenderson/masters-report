...
The numerical results in this section are interpreted through the contract stack
in Section~\ref{subsec:numerical-contract-stack}, the
well-balanced preservation checklist in Definition~\ref{def:well-balanced-checklist},
and the residual budget table in
...
discrepancies under the declared model, discretization, observation, boundary,
and metric choices, not causal attribution.
...
For interpretive accounting, and not as a causal decomposition, the
section-level split is
\begin{flalign*}
    \quad
    d_{u,j}
    =
...
\end{flalign*}
where the superscripts are contract tags only:
$\mathrm{cont}$ captures model+closure contributions (the equation contract);
$\mathrm{disc}$ captures discrete and evolution contributions; $\mathrm{obs}$ captures
the observation map; $\mathrm{bnd}$ captures fixed-area boundary approximation; and
$\mathrm{metric}$ captures metric reduction.
...
\begin{table}[!htb]
    ...
    \begin{tabular}{@{}l>{\raggedright\arraybackslash}p{0.45\linewidth}p{0.42\linewidth}@{}}
        \toprule
        \textbf{Metric family}
            & \textbf{Contract budget identity}
            & \textbf{Not identifiable from the retained comparison metrics} \\
        \midrule
        mean
            & $\overline d_u$, $\overline d_Q$
            & Separation of contract-layer means for the two scalar means.
            \\
        $L_1,L_2,L_\infty$
            & $D_{u,1},D_{u,2},D_{u,\infty},
            D_{Q,1},D_{Q,2}$
            & Layer-level attribution, including which layer dominates each
            absolute/RMS quantity.
            \\
        relative RMS
            & $D^{\mathrm{rel}}_{u,2}$
            & Sensitivity to the denominator norm convention and its impact
            on case comparability.
            \\
        \bottomrule
    \end{tabular}%
    }
...
operators. This interpretation remains conditional on Section~\ref{subsec:numerical-contract-stack}
and Definition~\ref{def:well-balanced-checklist}. It does not provide externally
