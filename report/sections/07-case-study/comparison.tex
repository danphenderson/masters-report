...
\begin{figure}[!htb]
    ...
    \caption{Axial physical-flow comparison under the plane-quadrature
    operator. Dashed curves are $Q_{1D}(z)=\pi q(z)$ and solid curves are
    $Q_{3D}(z)$ from the resolved velocity fields.}
...
\begin{table}[!htb]
    ...
    physical-flow and area-denominator contributions. Values are in cm/s.}
...
\begin{table}[!htb]
    ...
    the matched final-time sample. Values are in cm/s except for the axial
    location column in cm.}
...
\begin{figure}[H]
    ...
    \caption{Area-mean axial velocity comparison at $t=0.9995$ s. Solid curves
...
\begin{table}[!htb]
    ...
    resolved-velocity comparison. Flow discrepancies are in cm$^3$/s and
    velocity discrepancies are in cm/s.}
...
\begin{table}[!htb]
    ...
    in cm/s; physical flows and flow discrepancies are in cm$^3$/s.}
...
match: the retained record contains the final displacement field, not the
...
\begin{table}[!htb]
    ...
    measures. $D_{u,1}$ and $D_{u,2}$ are the mean absolute and RMS
    section-mean velocity discrepancies in cm/s; $D_{Q,1}$ is the mean absolute
    physical-flow discrepancy in cm$^3$/s.}
...
\begin{figure}[!htb]
    ...
    section-wise velocity discrepancy. Solid curves use the reference mesh
...
selected 1D pressure--area closure used in the reduced model.
...
\begin{figure}[!htb]
    \centering
    \captionsetup{aboveskip=2pt,belowskip=2pt}
    \figtikz{membrane-fsi-wall-profile}
    ...
    clamped membrane equation; the lower panel shows the resulting deformed
...
\begin{figure}[!htb]
    \centering
    \captionsetup{aboveskip=2pt,belowskip=2pt}
    \figtikz{membrane-fsi-fixed-point-history}
    ...
    successive coupled wall solves, reported in centimeters.}
...
\begin{table}[!htb]
    \centering
    \scriptsize
    \caption{Radial-profile promotion audit. The zero summary delta confirms
    ...
    \input{report/assets/tables/stenosis-comparison/radial_profile_audit.tex}
\end{table}

\subsubsection{Interpretation Limits}
\label{subsec:spatial-localization-discrepancies}

The result of this chapter is a controlled numerical validation experiment
against declared resolved comparators: a reference-coordinate velocity
comparison, a displacement-aware observation comparison, and a quasi-static
membrane-wall comparator. The evidence supports the implemented comparison
workflow and localizes final-time velocity discrepancies under the stated
operators. It does not establish clinical prediction, pressure-ratio accuracy,
or equivalence to a full transient FSI computation with matched boundary and
wall-history data.
