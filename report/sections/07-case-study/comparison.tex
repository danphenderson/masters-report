...
Appendix~\ref{app:code-and-ai-use} records the raw-input policy, regeneration
commands, and tracked table-fragment renderer for this comparison.
The same appendix also reports the broader source-artifact replication of
Canic et al.\ Section~4.1 after restoring the optional upstream 3D bundles; that
run is supplemental to this chapter's retained two-case comparator because it
uses the upstream article bundle set and records its own parameter and snapshot
audit in Tables~\ref{tab:canic-section41-summary} and
...
Table~\ref{tab:t1-metric-residual-budget}.
...
observation-operator comparison rather than a gross cut-geometry mismatch.
...
Interpretation of these summaries follows the residual budget in
Table~\ref{tab:t1-metric-residual-budget}; the rows support only aggregate
...
\paragraph{Residual-budget interpretation and inferential limits}
\label{subsec:metric-residual-budget-interpretation}
For interpretive accounting, rather than causal decomposition, write the
section-level discrepancies as
\begin{flalign*}
    \quad
    d_{u,j}
    =
    ...
    +d_{Q,j}^{\mathrm{obs}}
    +d_{Q,j}^{\mathrm{bnd}}
    +d_{Q,j}^{\mathrm{metric}}, &&
\end{flalign*}
where the superscripts indicate only the layer at which discrepancy enters the
comparison: $\mathrm{cont}$ for equation and closure choice, $\mathrm{disc}$ for
discrete evolution, $\mathrm{obs}$ for the observation rule, $\mathrm{bnd}$ for
the boundary approximation, and $\mathrm{metric}$ for the final aggregation.
...
\begin{table}[!htb]
    ...
    \label{tab:t1-metric-residual-budget}
    \resizebox{\textwidth}{!}{%
...
Radial distributions of axial velocity are not promoted into the main
comparison claim. The package audit now classifies section and radial-bin
closure on identical cuts, and the tracked radial-profile audit CSVs and
compact \TeX{} table are retained as supplemental reproducibility artifacts.
The manuscript retains only the section-mean axial-velocity comparison for
...
operators. The clean public source tree can rebuild the manuscript from the
tracked derived assets and can verify missing raw resolved-3D inputs as an
expected skip; raw-data-dependent asset regeneration requires restoring the
ignored XDMF/HDF5 inputs described in Appendix~\ref{app:code-and-ai-use}. After
those inputs are restored, the promoted Canic Section~4.1 source-artifact
workflow reproduces the Section~4.1 numerical comparison surfaces as derived
velocity, pressure, radial-postprocessing, and 3D-diagnostic assets, but it
still does not regenerate the article's raw transient 3D FSI solution. This
