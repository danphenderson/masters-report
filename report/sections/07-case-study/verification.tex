...
identifies the non-well-balanced limitation of the selected scheme, and
admissibility diagnostics report the boundary, CFL, positivity, and
conservation regime of the accepted comparison runs. Secondary checks for
...
selected comparison runs start from that rest state, but the MUSCL/Rusanov
scheme used here does not exactly preserve it. Boundary, CFL, positivity, and
conservation diagnostics then state whether accepted runs remained numerically
admissible. Self-convergence, stationary-Stokes projection, broad
rheology/profile sweeps, resolved-velocity benchmark stages, and nonselected DG
rows are secondary checks kept in Appendix~\ref{app:numerical-methods-details}.
...
The implementation now also includes an opt-in finite-volume
geometry-rest-balanced variant for bounded diagnostic grids. It retains the
MUSCL limiter and Rusanov-type dissipation but applies the dissipative area jump
to the perturbation $a-a_0$, with $a_0(z_i)=R_0(z_i)^2$, and replaces the wall
source by the corresponding discrete source that cancels the elastic flux
difference at the sampled rest state. In zero-forcing, zero-inlet runs this
preserves the declared geometry-rest state to roundoff in focused package tests
and in the same rest-drift CSV/\TeX{} workflow used below. This row is a
method-development diagnostic, not a retroactive change to the principal
MUSCL/Rusanov comparison results reported in the case study.
...
exact rest state, but the reported MUSCL/Rusanov realization does not preserve
that state exactly on the production grid. The zero-forcing rest test starts
the selected 23\% and 40\% stenosis geometries from the rest state with a
...
values are $0.04387$ and $0.07053\,\mathrm{cm}^3/\mathrm{s}$. The retained
refinement rows show the same localized defect decreasing through
$N=1600$ and $N=3200$: final $t=1$ s values are $0.002442$ and
$0.004028\,\mathrm{cm}^3/\mathrm{s}$ on $N=1600$, and $6.11\times10^{-4}$ and
$1.009\times10^{-3}\,\mathrm{cm}^3/\mathrm{s}$ on $N=3200$. The adjacent-grid
final-flow rates from $N=400$ through $N=3200$ are approximately
$1.98$--$2.00$ for both severities. The defect is therefore strongly localized
and strongly grid-decreasing, but it remains non-negligible on the production
comparison grid.

An additional method-sensitivity check asks whether this rest-state limitation
is only a peculiarity of the production MUSCL reconstruction. Table~\ref{tab:rest-method-sensitivity}
runs the same zero-inlet geometry-rest diagnostic at $N=400$ through the
available historical finite-volume methods and includes the 50\% stenosis
geometry as a higher-severity stress case. First-order fluxes are much more
diffusive, and WENO3 reduces the instantaneous residual and drift by more than
an order of magnitude, but the historical executable finite-volume comparator
rows still do not preserve the sampled geometry-rest state exactly. The
published Lax--Wendroff row is retained as a diagnostic-surface capability
boundary from that scratch run: its residual decomposition attempted a $t=0$
flux evaluation with zero native diagnostic timestep, while that flux requires a
positive native time step. Newly generated method-development diagnostics can
additionally select \texttt{fv-wb-geometry-rest}; those rows are expected to
report near-roundoff initial residual and drift for the declared rest family and
should be interpreted separately from the historical comparator table.
...
\begin{table}[!htb]
    ...
    \resizebox{\textwidth}{!}{%
    \input{report/assets/tables/verification/rest_method_sensitivity.tex}
    }
\end{table}
...
case study. It does not invalidate the later section-mean velocity comparison,
but it fixes the comparison's interpretation: the reported 1D--3D discrepancy is
a conditional diagnostic comparison carried out with a discretization whose
equilibrium preservation is known to be imperfect. The zero-inlet magnitudes do
not support a large through-flow interpretation, but the nonzero localized
residual rules out reading the production comparison as a clean production-grid
