...
checks for nonprincipal numerical variants remain in
...
(Proposition~\ref{prop:selected-rest-state-compatibility}). The historical
plain MUSCL/Rusanov row is not exactly well balanced for that state: its
...
finite-volume variant. Table~\ref{tab:rest-method-sensitivity} runs the same
...
this auxiliary grid, but it is not the selected comparison method and does not
