...
\subsection{Numerical Interpretation Chain}
\label{subsec:numerical-contract-stack}

The report uses the following common template for numerical claims:
\begin{flalign*}
    \quad
    \mathcal L_{\mathcal P}: \mathcal U\to\mathcal Y,\qquad
    M_{h,\theta_h}: \mathcal U \to \mathcal U_h\subset\mathcal U,\qquad
    \Psi_{\Delta t,h,\lambda_h}: \mathcal U_h \to \mathcal U_h,\\
    \quad
    \mathcal H_{h,\omega_h}: \mathcal U_h\to \mathcal Z_h,\qquad
    \mathcal R_{\mathrm{metric}}: \mathcal Z_h\to\mathbb R^k,\qquad
    Y_{h,\Delta t}
    =
    \mathcal R_{\mathrm{metric}}\!\left(
    \mathcal H_{h,\omega_h}\!\left(
    \Psi_{\Delta t,h,\lambda_h}\!\left(
    M_{h,\theta_h}(I_h(U_0;\theta_h))\right)\right)\right). &&
\end{flalign*}
Here $\mathcal U$ is the continuous state space, $\mathcal U_h\subset\mathcal U$
is the discretized state space, and $\mathcal Z_h$ is the declared observation
space (section means, section flows, pressure tuples, etc.).
The lifting $I_h:\mathcal U\to\mathcal U_h$
maps continuous initial data into the discrete state. This is an interpretive
template for comparing models, discretizations, observations, and metrics; it
is not an additional discretization method.

This comparison is interpretable only when all parts of the template are fixed:
\begin{enumerate}
    \item the model specification $\mathcal P$ and target equation
        $\mathcal L_{\mathcal P}(U)=0$;
    \item discretization controls $(\theta_h,\Delta t,h)$, initialization $I_h$,
        and semidiscrete realization;
    \item evolution controls $\lambda_h$ and corresponding admissible timestep
        assumptions;
    \item observation controls $\omega_h$ and shared output rule
        $\mathcal H_{h,\omega_h}$;
    \item metric definition $\mathcal R_{\mathrm{metric}}$, including denominator and
        weighting conventions.
\end{enumerate}

The resulting interpretive chain is
\[
    \mathcal L_{\mathcal P}\to
    M_{h,\theta_h}\to
    \Psi_{\Delta t,h,\lambda_h}\to
    \mathcal H_{h,\omega_h}\to
    \mathcal R_{\mathrm{metric}}.
\]

\subsection{Local Stencils and Coupling Maps}
\label{subsec:numerical-methods-stencil-contract}
...
numerical issues have to be kept separate. First, when the continuous
...
stabilization mechanism. Third, advective stabilization such as SUPG addresses
...
evidence together \cite{GaldiEtAl2008HemodynamicalFlows}. Gridap-based tests in
this report provide formulation-level evidence for continuous
P2-velocity/P1-pressure routes. They check the local mathematical
specification; they do not constitute a manuscript-grade reproduction or
...
When the wall moves, the numerical specification also includes the fluid--structure
...
\begin{definition}[Well-balanced preservation checklist]
\label{def:well-balanced-checklist}
For a target rest family $\mathcal E\subset\mathcal U$ and shared sampling
interval $\mathcal T$, a well-balanced claim requires the following
specification:
\begin{enumerate}
    \item the model specification and source balance define $\mathcal E$;
    \item boundary realizations are constrained to the same equilibrium family;
    \item the discrete maps $M_{h,\theta_h}$ and $\Psi_{\Delta t,h,\lambda_h}$
        are used with the same equilibrium assumptions;
    \item the same observation operator $\mathcal H_{h,\omega_h}$ is used across
        compared runs;
    \item reported residuals use a fixed metric operator
        $\mathcal R_{\mathrm{metric}}$ and shared section index set.
\end{enumerate}
Appendices~\ref{app:num-current-solver-operator},
\ref{app:num-boundary-state-realization}, and
\ref{app:num-full-rest-state-drift-grid} provide the implementation checks.
\end{definition}
...
history, timestep, and observable satisfy declared tolerances for the intended
...
error. The section geometry used by the observation operator must also be declared:
...
\begin{table}[!htb]
    ...
    \begin{tabularx}{\textwidth}{@{}
        ...
        \midrule
        Implementation matches a manufactured smooth solution
        ...
        Two schemes agree
            & Shared benchmark, matched model specifications, and comparable
            output
            quantities.
            & Correctness if the shared model specification is incomplete. \\
