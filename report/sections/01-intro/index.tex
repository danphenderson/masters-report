...
being compared. A three-dimensional (3D) computational fluid dynamics (CFD)
simulation of the Navier--Stokes system, a reduced one-dimensional (1D)
area--flow model, and a lumped zero-dimensional (0D) outlet network therefore
do not answer the same mathematical question. Anatomical narrowing and
...
surrogates, and observation operators. Appendix~\ref{app:code-and-ai-use}
records the reproducibility metadata for the source inventory and generated
evidence matrices; the main text uses those records only to support bounded
claims.
...
observables; and software or repository records support reproducibility rather
...
clinical validation result. It first establishes the continuum vocabulary,
...
stenosis case study. The case uses \path{StenoticHemodynamics} as a worked
example of a reduced model contract, closure choices, evidence records, and a
1D--3D velocity observation operator. Its role is methodological: it shows how
the review concepts constrain interpretation without converting the report into
a software or clinical-validation document.
