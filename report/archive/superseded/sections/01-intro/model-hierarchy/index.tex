\subsection{Reduced-Order Blood-Flow Models}
...
three-dimensional continuum problem; the useful reduced-order sequence is then
the distributed 1D reduction, the lumped 0D/network closure, and any
intermediate 2D diagnostic tier. Dimension, wall treatment, rheology, boundary
data, coupling laws, output operators, and numerical realization remain
separate modeling choices. This report selects the 1D area--flow model as the
reduced forward map; 3D, 0D, 2D, and multiscale models supply comparison,
boundary-condition, diagnostic, and coupling vocabulary.
...
\input{report/sections/01-intro/model-hierarchy/1d-model}
\input{report/sections/01-intro/model-hierarchy/0d-model}
\input{report/sections/01-intro/model-hierarchy/2d-model}

\subsection{Geometrical Multiscale Coupling}
\label{subsec:geometrical-multiscale-coupling}

\input{report/sections/01-intro/model-hierarchy/multiscale-model}
